...
if (substring(getInfo("os.name"), 0, 3) == "Win") {
	cellposeDir = "C:\\Users\\Public\\Documents\\Anaconda_MU\\envs\\cellpose"; //"C:\\Users\\Public\\Documents\\Anaconda_MU\\envs\\cellpose";} //C:\\Users\\uqshongy\\.conda\\envs\\cellpose
}

print("Volumetric Analysis of endosomal vesicles (inverse fluorescent imaging)");
print("Scripted by Hongyu (Stuart) Shen with help of Dr. Nicholas D. Condon");
print("and Dr. Claudia J Stocks for her moral support");
...
print("Version 0.9.5C (cellpose) | Cellpose 3.0,  Model: cyto3");
...
Dialog.create("Setup");
	...
	Dialog.setInsets(0,0,15);
	Dialog.addString("Vesicle Area (µm^2) :", "0.5-25");
	Dialog.addCheckbox("Cellpose", true);
//	Dialog.setInsets(0,0,15);
	Dialog.addCheckbox("Custom Model", false);
	Dialog.addFile("Choose your custom model:", "I:/HDRSTUART-Q6156/Scripting/BMM_Custom_models/CP_BMM_Enhanced_2");
	Dialog.setInsets(0,0,15);
	Dialog.addString("Cell diameter:", "250");
	Dialog.addToSameRow();
	Dialog.addString("Cell probability:", "-4");
	Dialog.addToSameRow();
...
for (i = 0; i < list.length; i++) {
	if (endsWith(list[i], ext)) {
		...
		//Dragonfly weird behaviour compensation
		run("Duplicate...", "duplicate range=2-" + nSlices - 2);
		...
		if (cellpose == true) {
			modelPath = "model=cyto3 model_path=";
			if (customModel == true) {
 ...
 * Total vesicular volume does not equals to the sum of individuals when vesicles has layer of 1 (Noise in total value)
 */