...
print("nunununununununununununununununununununununununununununununun\n");
	...
	if (endsWith(list[i], ext)) {
		...
		for (k = 0; k < RoiManager.size; k ++) {
			...
			for (l = 2; l < nResults; l ++) {
				if (extrasChoice == "DragonFly de-noise") {indVol = indVol + (getResult("IntDen", l) + 2) * depth + ", ";}
				else {indVol = indVol + getResult("IntDen", l) * depth + ", ";}
				numVes = l - 1;
			}
			if (nResults > 2) {concat = concat + indVol;}
			
			//Output file
			print(Results, ",,," + fileCount + "_" + k + 1 + "," + cellVol + "," + veslVol + "," + numVes + "," + indVol);
		}
...
print(Results, "Concatenated Individual Volume: ," + concat);
