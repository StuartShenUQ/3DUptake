...
if (substring(getInfo("os.name"), 0, 1) == "W") {cellposeDir = "C:" + fs + "Users" + fs + "uqshongy" + fs + ".conda" + fs + "envs" + fs + "cellpose";}
	...
	if (endsWith(list[i], ext)) {
		...
		for (k = 0; k < RoiManager.size; k ++) {
			...
			for (l = 2; l < nResults; l ++) {
				pxiA = getResult("Area", l);
				pixV = getResult("IntDen", l);
				//Estimated volume based on area
				estVol = estVol + (4/3 * PI * pow(sqrt(pxiA/PI),3)) * depth + ", ";
				
				//Measured volume
				if (extrasChoice == "DragonFly de-noise") {mesVol = mesVol + (pixV + 2) * depth + ", ";}
				else {mesVol = mesVol + pixV * depth + ", ";}
				numVes = l - 1;
			}
