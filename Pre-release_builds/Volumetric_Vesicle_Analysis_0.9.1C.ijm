...
print("Version 0.9C (cellpose) | Cellpose 3.0,  Model: cyto3");
...
Dialog.create("Setup");
	...
	Dialog.setInsets(0,0,15);
	Dialog.addString("Cell diameter:", "200");
	Dialog.addToSameRow();
...
refPath = outputPath + "References" + year + "-" + (month+1) + "-" + day + "_" + hour + "." + min + fs;
...
for (i = 0; i < list.length; i++) {
	if (endsWith(list[i], ext)) {
		...
		if (cellpose == true) {
			...
			run("Cellpose ...", "env_path=" + cellposeDir + " env_type=conda model=cyto3 model_path= diameter=" + cellD + " ch1=0 ch2=-1 additional_flags=[--use_gpu, --cellprob_threshold, " + cellProb + ", --flow_threshold, " + flowTh + "]");
