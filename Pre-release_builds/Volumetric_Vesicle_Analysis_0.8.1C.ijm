...
print("Version 0.8.1C (cellpose) | Cellpose version 2, Model: cyto2");
...
function progressBar(a, b, c) {
	...
	for (i = 0; i <= p; i ++) {
		if (i > pp) {
			print("\\Update" + c + ":" + "[" + substring(barL, 0, i) + substring(barR, i, range) + "] " + round(a/b*100) + "%");
			wait(20);
		}
	}
}
...
for (i = 0; i < list.length; i++) {
	if (endsWith(list[i], ext)) {
		...
		if (cellpose == true) {
			...
			index = index + 2;
			//print(RoiManager.size + " cell(s) found\n");
		...
		if (refMode == true) {
			...
			if (cellpose == true) {
				...
				run("Merge Channels...", "c1=SUM_Cells c2=[SUM_Vesicles] create ignore");
			}
