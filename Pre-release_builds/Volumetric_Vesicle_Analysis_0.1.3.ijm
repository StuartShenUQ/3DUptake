...
for (i = 0; i < list.length; i++) {
	if (endsWith(list[i], ext)){
		...
		run("Measure");
		cellVol = getResult("IntDen");
		
		selectImage("Mask of Result of MPS");
		run("Subtract...", "value=254 stack");
		run("Z Project...", "projection=[Sum Slices]");
		run("Measure");
		mpslVol = getResult("IntDen");
		run("Duplicate...", "title=MPScount duplicate");
		setAutoThreshold("Huang dark no-reset");
		run("Analyze Particles...", "size=0.5-50 circularity=0.8-1.00 show=Masks clear stack");
		//count how many MPS is in the frame
		//mpsNum = getResult(????);
...
Results = File.open(inputPath + "Results_" + "_" + year + "-" + (month+1) + "-" + day + "_at_" + hour + "." + min + ".xls");
