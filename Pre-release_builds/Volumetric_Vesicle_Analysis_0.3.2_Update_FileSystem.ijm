...
for (i = 0; i < list.length; i++) {
	if (endsWith(list[i], ext)){totalFilecount = totalFilecount + 1;}
...
for (i = 0; i < list.length; i++) {
	if (endsWith(list[i], ext)){
		...
		print("Processing Image " + fileCount + "/" + totalFilecount);
		run("Clear Results");
		showProgress(i+1, totalFilecount);
		...
		//Output file
		print(Results, fileCount + "," + File.getNameWithoutExtension(list[i]) + "," + cellVol + "," + mpslVol + "," + numVes + "," + indVol);
